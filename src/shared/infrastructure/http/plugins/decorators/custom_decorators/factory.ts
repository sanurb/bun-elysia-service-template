...
import type { ElysiaRoute, RouteOptions } from '../types';
...
const createRouteMethod =
  (method: string) =>
  (
    path = '/',
    { config, ...hook }: RouteOptions = { config: { allowMeta: false } }
  ) =>
    getMethodFunction(method, path, { config, ...hook });
