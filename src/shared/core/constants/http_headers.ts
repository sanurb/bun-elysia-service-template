/**
 * @fileoverview
 * HTTP header name constants for both request and response.
 *
 * Defines:
 *  - `RequestHeader` and `RequestHeaderRecord`
 *  - `ResponseHeader` and `ResponseHeaderRecord`
 */
...
/**
 * All valid request‐header names, including any user‐added ones.
 */
export type RequestHeader = (typeof RequestHeader)[keyof typeof RequestHeader];

/**
 * A mapping from request‐header names to their string values.
 */
export type RequestHeaderRecord = Partial<Record<RequestHeader, string>>;
...
export const ResponseHeader = {
  ...
  /**
   * Last modification date of the resource, used for conditional requests.
   *
  ...
  /**
   * A unique string identifying the version of the resource. Used for ETag-based conditionals.
   *
...
/**
 * All valid response‐header names, including any user‐added ones.
 */
